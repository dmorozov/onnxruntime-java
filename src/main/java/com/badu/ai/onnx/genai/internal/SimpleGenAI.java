 ...
 *
 * <ul>
 * <li>Create an instance of the class with the path to the model. The path should also contain the
 * GenAI configuration files.
 ...
 *
 * <p>
 * The listener is used as a callback mechanism so that tokens can be used as they are generated. It
 * should be an instance of a type that implements the Consumer&lt;String&gt; interface.
 */
public class SimpleGenAI implements AutoCloseable {
   ...
   *
   * <p>
   * NOTE: This only handles a single sequence of input (i.e. a single prompt which equates to batch
   * size of 1)
   *
   ...
   *        generation will be blocked until the listener's `accept` method returns. `listener` will
   *        be called within the token generation loop and these calls will be made sequentially,
   *        not concurrently.
    ...
    try {
      ...
      if (listener != null) {
        try (TokenizerStream stream = tokenizer.createStream();
          ...
          // isDone)
          Sequences inputSeqs = tokenizer.encode(prompt);
          inputTokenCount = inputSeqs.getSequence(0).length;
          generator.appendTokenSequences(inputSeqs);
      ...
      if (output_ids.length > inputTokenCount) {
        int[] generatedTokens = new int[output_ids.length - inputTokenCount];
        System.arraycopy(output_ids, inputTokenCount, generatedTokens, 0, generatedTokens.length);
        result = tokenizer.decode(generatedTokens);
      } else {
